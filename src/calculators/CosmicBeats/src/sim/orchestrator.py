...
from ..nodes.topology import Topology
from ..models.imodel import IModel
from ..sim.nodeinits import nodeInitDictionary
from ..sim.modelinits import modelInitDictionary


class Orchestrator:
    '''
    This class orchestrates the simulation environment. The main jobs are, reading the config file, creating the
    nodes accordingly having asked models, resolving the dependencies of models, allocating resources (e.g., thread,
    VM) to the nodes.
    '''
        ...
        for _topologyConfig in self.__configdata.topologies:
            ...
            for _nodeConfig in _topologyConfig.nodes:
                try:
                    ...
                    # initialize the node by looking at the node init dictionary. User mentions the iname (
                    # implementation class name of a node) of the node, and we try to find the corresponding
                    # initialization function in the dictionary
                    ...
                    self._add_Models(_newNode, _modelConfig)

                    # Models have been added to the node. Now, we can add the  node to the topology
                    _topologyIns.add_Node(_newNode)
                except:
                    raise Exception(
                        f"[Simulator Exception] Error in initializing node of topology: {str(_topologyConfig.id)},"
                        f" node: {str(_nodeConfig.nodeid)}")
        ...
        for _thisModelDetails in _modelDetails:
            ...
            if _isThisNodeSupported:
                ...
                if (_modelNameSet != set() and
                    ...
                    print(
                        f"[Simulator Warning] Model {_modelIns.iName} has been added to node {str(_nodeInstance.nodeID)} multiple times.")
                else:
                ...
                raise Exception(
                    "[Simulator Exception] Model {_modelIns.iName} does not support node {_nodeInstance.iName}")
        ...
        if not _dependencyResolved:
            ...
            for _modelToBeChecked in _tempModelList:
                        ...
                        for _dependencyModelClassName in _dependencySublist:
                            ...
                            for _aModelFromTheList in _tempModelList:
                                ...
                                if (_aModelFromTheList.iName == _dependencyModelClassName):
                                    ...
                                    break
                            # if the dependency is resolved for this sublist, we don't need to check further,
                            # so break the loop
                ...
                if not _dependencyResolved:
                    ...
                    raise Exception(
                        f"[Simulator Exception] Model {_modelToBeChecked.iName} has dependency mismatch inside node ID: {str(_nodeInstance.nodeID)} Model wanted: {str(_modelNameSet)}")
    ...
    def get_SimStartTime(self):
        return self.__simStartTime

    def get_SimEndTime(self):
        return self.__simEndTime
