...
from ...utils import Location, Time
...
class ModelOrbit(IModel):
    ...
    @staticmethod
    def max_visible_distance(h1, h2, R=6371000):
        """
        Berechnet die maximale Sichtweite zwischen zwei Punkten in Höhen h1 und h2
        über der Erdoberfläche mit Radius R.

        Alle Entfernungen sind in Metern.
        """
        d = math.sqrt((R + h1) ** 2 - R ** 2) + math.sqrt((R + h2) ** 2 - R ** 2)
        return d

    def check_visibility(self, sat, t, _tol):
        """
        This method checks if sat is visible from the position of ownernode including a tolerance _tol.

        @return
            Visibility as boolean
        """
        distance = self.__ownernode.get_Position(t).get_distance(sat.get_Position(t))
        # max_distance = self.max_visible_distance(posA.to_lat_long()[2], posB.to_lat_long()[2]) - _tol

        # return distance < max_distance
        # return distance < 5000000  # this is a reasonable approximation for two satellites in Starlink
        return distance < 7880467  # this is a reasonable approx for two satellites in OneWeb w/o atmosphere crossing

    def find_exact_second(self, sat, t, visible, interval, _tol):
        """
        This method finds the exact second in which the visibility
        between ownernode and sat changed in the interval before t.

        @return
            The exact time of visibility change (utils.Time)
        """

        upper_bound = t.copy()
        lower_bound = upper_bound.copy()
        lower_bound.add_seconds(-interval)
        current_interval = interval

        for _ in range(int(math.log(interval, 2) + 1)):  # maximale Iterationen, um eine Sekunde genau zu finden
            current_interval = current_interval / 2
            mid_point = lower_bound.copy()
            mid_point.add_seconds(current_interval)
            mid_point_visible = self.check_visibility(sat, mid_point, _tol)

            if mid_point_visible == visible:
                upper_bound = mid_point
            else:
                lower_bound = mid_point

            if upper_bound.difference_in_seconds(lower_bound) <= 1:  # ausreichend kleines Intervall (≤ 1 Sekunde)
                break

        return upper_bound.round_to_nearest_second() if visible else lower_bound.round_to_nearest_second()

    def __get_SatellitePasses(self, **_kwargs) -> 'list[(Time, Time)]':
        """
        This method calculates the start and end times of the passes
        between two satellites, considering a tolerance.

        @param[in] _kwargs
            Keyworded arguments passed to the method
            @key _satelliteB
                The other satellite for which the passes are to be calculated.
            @key _start
                The start time of the passes (utils.Time)
            @key _end
                The end time of the passes (utils.Time)
            @key _tol
                The tolerance a satellite has to be closer than the theoretical visibility distance in m (float)
        @return
            List of tuples of start and end time of the passes.
        """

        interval = 600  # search interval in seconds
        satelliteB = _kwargs['_satelliteB']
        _start = _kwargs['_start']
        _end = _kwargs['_end']
        _tol = _kwargs['_tol']
        ...
        current_time = _start.copy()
        currently_visible = self.check_visibility(satelliteB, current_time, _tol)
        if currently_visible:
            # If this is the start of a new pass, add a new tuple to the passes list
            passes.append((current_time.copy(), None))
        while current_time <= _end:

            visible = self.check_visibility(satelliteB, current_time, _tol)

            if visible != currently_visible:
                exact_time = self.find_exact_second(satelliteB, current_time, visible, interval, _tol)
                if visible:
                    # If this is the start of a new pass, add a new tuple to the passes list
                    passes.append((exact_time.copy(), None))
                else:
                    # If the satellites can no longer "see" each other, end the last pass
                    passes[-1] = (passes[-1][0], exact_time.copy())
                currently_visible = visible

            current_time.add_seconds(interval)

        if currently_visible:
            # If this is the start of a new pass, add a new tuple to the passes list
            passes[-1] = (passes[-1][0], current_time)

        return passes

    def __get_SatelliteNeighbour(self, **_kwargs) -> bool:
    ...
    def __get_absolute_Velocity(self, **kwargs):
        x, y, z = self.__get_Velocity(**kwargs)
    ...
    __apiHandlerDictionary = {
        ...
        "get_SatellitePasses": __get_SatellitePasses,
        "get_SatelliteNeighbour": __get_SatelliteNeighbour,
        "get_Position": __get_Position,
        "get_Velocity": __get_Velocity,
        "get_absolute_Velocity": __get_absolute_Velocity,
    ...
    def __init__(
        ...
        self.__ephem = load("https://ssd.jpl.nasa.gov/ftp/eph/planets/bsp/de440.bsp") #ephemeris file. This is a binary file that contains the positions of the earth and the sun.
...
def init_ModelOrbit(
    ...
    return ModelOrbit(_ownernodeins, _alwaysCalc)
